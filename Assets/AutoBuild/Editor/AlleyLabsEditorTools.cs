...
public class AlleyLabsEditorTools {
	...
	public static bool BeginFoldOut(string text,bool foldOut, bool endSpace=true){
		...
		GUIStyle myStyle = new GUIStyle ("dragtab");
		myStyle.margin = new RectOffset (3, 3, 10, 2);
		if ( !GUILayout.Toggle(true,text,myStyle)){
			foldOut=!foldOut;
		}
		
		if (!foldOut && endSpace)GUILayout.Space(10f);
		return foldOut;
	}
